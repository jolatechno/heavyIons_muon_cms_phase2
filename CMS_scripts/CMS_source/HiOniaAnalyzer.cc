...
void HiOniaAnalyzer::fillTreeMuon(const pat::Muon* muon, int iType) {
  ...
  if (muon != NULL) {
    ...
    if (!_theMinimumFlag) {
      ...
      Reco_mu_nMatches[Reco_mu_size] = muon->numberOfMatches();
      Reco_muGEMquality[Reco_mu_size] = muon::isGoodMuon(*muon, muon::GEMMuonArbitrated);
...
void HiOniaAnalyzer::InitTree() {
  ...
  if (!_theMinimumFlag) {
    ...
    myTree->Branch("Reco_mu_nMatches", Reco_mu_nMatches, "Reco_mu_nMatches[Reco_mu_size]/I");
    myTree->Branch("Reco_muGEMquality", Reco_muGEMquality, "Reco_muGEMquality[Reco_mu_size]/O");
